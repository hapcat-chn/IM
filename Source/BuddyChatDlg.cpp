...
CBuddyChatDlg::~CBuddyChatDlg(void)
{
 	m_lpFMGClient->m_videoRecord.chatClosed(m_UserId,this);
	LOG_INFO("chat closed!");
}
...
void CBuddyChatDlg::OnVoiceMsg(UINT uNotifyCode, int nID, CWindow wndCtl){
	...
	if( 
		m_VideoSelDlg.m_RadioState == RADIO_DLG_STATE_IDLE
			&& 
		m_lpFMGClient->m_videoRecord.queryRadioRes() == FALSE 
	  )
	{
		RichEdit_ReplaceSel(m_richRecv.m_hWnd, _T("                                            			☆请检查录音设备后重试☆\r\n"), 
	...
	if(m_lpFMGClient != NULL){
		...
		int ret = -1;
		if(m_VideoSelDlg.m_RadioState == RADIO_DLG_STATE_FINISH)	
		{
...
void CBuddyChatDlg::OnBtn_Send(UINT uNotifyCode, int nID, CWindow wndCtl)
{
	...
	tstring strText;
	RichEdit_GetText(m_richSend.m_hWnd, strText);//获取文字和图片等信息
	if (strText.empty())
	{
...
BOOL CBuddyChatDlg::InitMidToolBar()
{
	...
	m_tbMid.SetItemSize(nIndex, 30, 27);
	m_tbMid.SetItemPadding(nIndex, 1);
	m_tbMid.SetItemToolTipText(nIndex, _T("发送语音消息"));
	m_tbMid.SetItemBgPic(nIndex, NULL, _T("aio_toolbar_highligh.png"), 
		_T("aio_toolbar_down.png"), CRect(3,3,3,3));
	m_tbMid.SetItemIconPic(nIndex, _T("MidToolBar\\aio_quickbar_voice.png"));
